...
class catalogo extends CI_Controller {
    ...
    public function estados() {
        ...
        echo json_encode($rec_ests->result(), JSON_UNESCAPED_UNICODE);
    }
    ...
    public function colonias() {
        ...
        if ($municipio != false) {
            /* $rec_cols = Colonia::where('col_clavemunicipio', '=', $municipio)
              ->get(); */
            $query = 'select * from colonias where col_clavemunicipio = %s';
            $rec_cols = $this->db->query(sprintf($query, $municipio));
        }
